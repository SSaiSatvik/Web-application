...
export default function Follower() {
    ...
    async function test()
    {
        const response=await fetch("http://localhost:4000/ffs/getfollowers",
        {
            method:"POST",
            headers:{
        ...
        {
            console.log("didn't got followers list")
        }
            ...
            <div className='container'>
                <div className='row'>
                    <div className='col-md-12'>
                        <h2 className='display-4 text-center'>Followers List</h2>
                    </div>
                </div>
                <div className='list'>
                    {varff && (
                        ff && ff.map((name) => (<Indiviual name={name.follower}/>))
                    )}
                    {!varff && <h4>there is no followers</h4>}
                </div>
            </div>
        </div>
    </React.Fragment>
...
function Indiviual({name})
{
    const handle = async() => {
        ...
        const response=await fetch("http://localhost:4000/ffs/deleteff",
        {
            ...
            body: JSON.stringify({following:uname,follower:name})
        });
